...
myvals=function(s1,s2,from) {
  ...
  out<-list(
    co_intigration=as.numeric(coin),
    correlation_coeff=as.numeric(corre),
    volulme_1=mean(Vo(getSymbols(s1,auto.assign=FALSE,from=from))),
    volulme_2=mean(Vo(getSymbols(s2,auto.assign=FALSE,from=from)))
  )
...
value_sub<-subset(value,subset = co_intigration<0.04
...
pairs.trade(pair_stk_1,pair_stk_2)

par(mfrow=c(2,1))
mycoin(pair_stk_1,pair_stk_2,from = "2015-10-01")
mycor(pair_stk_1,pair_stk_2,from = "2015-10-01")
par(mfrow=c(1,1))
