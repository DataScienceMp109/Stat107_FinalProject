...
VWAP_SMA<-function(a,b,ticker,from,to){
  # startdate=" 2015-01-01"
  # enddate = "2016-08-30"
  # malength=10
  # ticker="AAPL"
  
  v<-getSymbols(ticker, from=from,to=to,auto.assign=FALSE)
  # v <- adjustOHLC(v)
...
mk_ret<-getSymbols('SPY',from=start,to=end,,auto.assign=FALSE)
plot(Ad(mk_ret))
...
mkt_ret<-getSymbols('SPY',from=start,to=end,auto.assign = FALSE)
plot(Ad(mkt_ret))
